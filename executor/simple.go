...
func (info *resourceOptionsInfo) loadResourceOptions(userResource []*ast.ResourceOption) error {
	for _, option := range userResource {
		...
		case ast.MaxQueriesPerHour:
			info.maxQueriesPerHour = mathutil.Min(option.Count, math.MaxInt16)
		case ast.MaxUpdatesPerHour:
			info.maxUpdatesPerHour = mathutil.Min(option.Count, math.MaxInt16)
		case ast.MaxConnectionsPerHour:
			info.maxConnectionsPerHour = mathutil.Min(option.Count, math.MaxInt16)
		case ast.MaxUserConnections:
			info.maxUserConnections = mathutil.Min(option.Count, math.MaxInt16)
		}
	}
	return nil
}
...
func (e *SimpleExec) executeCreateUser(ctx context.Context, s *ast.CreateUserStmt) error {
	...
	for _, spec := range s.Specs {
		...
		sqlexec.MustFormatSQL(sql, valueTemplate, hostName, spec.User.Username, pwd, authPlugin, userAttributesStr, plOptions.lockAccount, recordTokenIssuer, plOptions.passwordExpired, plOptions.passwordLifetime, userResource.maxUserConnections)
